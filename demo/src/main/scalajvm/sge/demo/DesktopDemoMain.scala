...
/** JVM desktop entry point for the SGE demo.
  *
  * Uses Panama FFM to load GLFW, ANGLE, and miniaudio shared libraries, then launches the demo application. The
  * DemoApp receives the Sge context via [[SgeAware.sgeAvailable]] before `create()` is called.
  */
