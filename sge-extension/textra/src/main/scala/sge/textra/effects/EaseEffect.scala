 ...
 * Covenant-baseline-methods: DEFAULT_DISTANCE,DEFAULT_EXTENT,EaseEffect,distance,elastic,extent,interpolatedValue,interpolation,onApply,progress,realExtent,timePassed,timePassedByGlyphIndex,y
 * Covenant-source-reference: com/github/tommyettinger/textra/effects/EaseEffect.java
...
class EaseEffect(label: TypingLabel, params: Array[String]) extends Effect(label) {
  private val DEFAULT_DISTANCE       = 0.15f
  private val DEFAULT_EXTENT         = 0.075f
  ...
  override protected def onApply(glyph: Long, localIndex: Int, globalIndex: Int, delta: Float): Unit = {
    ...
    val y                 = label.getLineHeight(globalIndex) * distance * interpolatedValue * DEFAULT_DISTANCE
