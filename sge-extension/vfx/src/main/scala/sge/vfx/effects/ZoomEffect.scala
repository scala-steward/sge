 ...
 * Covenant-baseline-methods: ZoomEffect,_originX,_originY,_zoom,originX,originY,ox,oy,rebind,render,setOrigin,zoom,zoom_
 * Covenant-source-reference: com/crashinvaders/vfx/effects/ZoomEffect.java
...
class ZoomEffect(using Sge)
    extends ShaderVfxEffect(
      VfxGLUtils.compileShader(
        Sge().files.classpath("sge/vfx/shaders/zoom.vert"),
        Sge().files.classpath("sge/vfx/shaders/zoom.frag")
      )
    )
    with ChainVfxEffect {
  ...
  def setOrigin(ox: Float, oy: Float): Unit = {
    ...
    _originY = oy
    program.bind()
    program.setUniformf("u_offsetX", ox)
    program.setUniformf("u_offsetY", oy)
    Sge().graphics.gl20.glUseProgram(0)
  }
