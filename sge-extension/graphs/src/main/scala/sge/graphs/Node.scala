...
/** Vertex node in the graph, wrapping a user object of type V.
  * Also carries mutable algorithm state (distance, prev, etc.) for search algorithms.
  */
class Node[V](
    val obj: V,
  ...
  var inEdges: InternalArray[Connection[V]] = if (trackInEdges) InternalArray[Connection[V]]() else null.asInstanceOf[InternalArray[Connection[V]]] // @nowarn — null used for optional in-edges tracking (matching original design)
