 ...
 *
 * Scala Native pin of the Double.toString-divergent numeric tokens for ISS-513. Scala
 * Native's java.lang.Double.toString matches the JVM (and therefore gdx's
 * JsonValue.asStringArray, JsonValue.java:398): 1e2 → "100.0", -0.0 → "-0.0". The Scala.js
 * copy of this suite pins the divergent "100"/"0" Scala.js output instead.
 */
package sge
package gltf
package loaders
...
class BlenderShapeKeysNumberFormatSuite extends munit.FunSuite {
  ...
  test("Native: exponent token 1e2 → \"100.0\" via Double.toString (JsonValue.java:398, matches gdx)") {
  ...
  test("Native: -0.0 token → \"-0.0\" via Double.toString (JsonValue.java:398, matches gdx)") {
  ...
  test(
    "Native E2E: 1e2 through the codec → \"100.0\" (BigDecimal.toString = \"1E+2\", contains '+'; Double.toString = \"100.0\")"
  ) {
  ...
  test(
    "Native E2E: -0.0 through the codec → \"0.0\" (readBigDecimal drops the negative-zero sign; Double.toString(0.0) = \"0.0\")"
  ) {
  ...
  test(
    "Native E2E: 40-digit integer → \"1.2345678901234568E39\" (>34-sig-digit codec rounding double-rendered; diverges from gdx's verbatim token)"
  ) {
