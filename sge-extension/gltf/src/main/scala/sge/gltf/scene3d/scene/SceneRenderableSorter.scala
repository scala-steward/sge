...
class SceneRenderableSorter extends RenderableSorter with Comparator[Renderable] {
  ...
  override def compare(o1: Renderable, o2: Renderable): Int = {
    ...
    val h1 = o1.userData match {
      case h: Nullable.Impl[?] => h.fold(null: SceneRenderableSorter.Hints) {
    ...
    if (h1 ne h2) { // @nowarn — identity comparison
      if (h1 == SceneRenderableSorter.Hints.OPAQUE_LAST) {
        return if (b2) -1 else 1 // @nowarn — early return
      }
      if (h2 == SceneRenderableSorter.Hints.OPAQUE_LAST) {
        return if (b1) 1 else -1 // @nowarn — early return
      }
    }
    ...
    if (!b1 && !b2) {
      ...
      if (shaderCompare != 0) return shaderCompare // @nowarn
      ...
      if (envCompare != 0) return envCompare // @nowarn
      ...
      if (materialCompare != 0) return materialCompare // @nowarn

      val meshCompare = compareIdentity(o1.meshPart.mesh, o2.meshPart.mesh)
      if (meshCompare != 0) return meshCompare // @nowarn
    } else if (b1 && !b2) {
      return 1 // @nowarn
    } else if (!b1 && b2) {
      return -1 // @nowarn
