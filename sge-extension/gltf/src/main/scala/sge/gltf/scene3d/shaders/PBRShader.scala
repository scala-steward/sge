...
class PBRShader(
  ...
  prefix:     String
)(using sge.Sge) extends DefaultShader(renderable, config, prefix) {
  ...
  private def countShadowCascades(renderable: Renderable): Int = {
    if (renderable.environment.isDefined) {
      renderable.environment.get.getAs[CascadeShadowMapAttribute](CascadeShadowMapAttribute.Type).map { csm =>
  ...
  override def canRender(renderable: Renderable): Boolean = {
    ...
    if (tcmMask != this.textureCoordinateMapMask) return false // @nowarn
    if (this.morphTargetsMask != computeMorphTargetsMask(renderable)) return false // @nowarn
    if (this.vertexColorLayers != computeVertexColorLayers(renderable)) return false // @nowarn
    if (this.cascadeCount != countShadowCascades(renderable)) return false // @nowarn
  ...
  override def render(renderable: Renderable, combinedAttributes: Attributes): Unit = {
    program.foreach { p =>
      if (u_mipmapScale != UniformLocation.notFound) {
        ...
        val mipmapFactor = specularEnv.map { env =>
...
object PBRShader {
  ...
  val baseColorTextureSetter: BaseShader.Setter = new BaseShader.LocalSetter {
    override def set(shader: BaseShader, inputID: Int, renderable: Renderable, combinedAttributes: Attributes): Unit = {
      val unit = shader.context.get.textureBinder.bind(
        combinedAttributes.getAs[TextureAttribute](PBRTextureAttribute.BaseColorTexture).get.textureDescription)
  ...
  val emissiveTextureSetter: BaseShader.Setter = new BaseShader.LocalSetter {
    override def set(shader: BaseShader, inputID: Int, renderable: Renderable, combinedAttributes: Attributes): Unit = {
      val unit = shader.context.get.textureBinder.bind(
        combinedAttributes.getAs[TextureAttribute](PBRTextureAttribute.EmissiveTexture).get.textureDescription)
      PBRShaderUtils.setInt(shader, inputID, unit)
    }
  }
  ...
  val normalTextureSetter: BaseShader.Setter = new BaseShader.LocalSetter {
    override def set(shader: BaseShader, inputID: Int, renderable: Renderable, combinedAttributes: Attributes): Unit = {
      val unit = shader.context.get.textureBinder.bind(
        combinedAttributes.getAs[TextureAttribute](PBRTextureAttribute.NormalTexture).get.textureDescription)
      PBRShaderUtils.setInt(shader, inputID, unit)
    }
  }
  ...
  val metallicRoughnessTextureSetter: BaseShader.Setter = new BaseShader.LocalSetter {
    override def set(shader: BaseShader, inputID: Int, renderable: Renderable, combinedAttributes: Attributes): Unit = {
      val unit = shader.context.get.textureBinder.bind(
        combinedAttributes.getAs[TextureAttribute](PBRTextureAttribute.MetallicRoughnessTexture).get.textureDescription)
      PBRShaderUtils.setInt(shader, inputID, unit)
    }
  }
  ...
  val occlusionTextureSetter: BaseShader.Setter = new BaseShader.LocalSetter {
    override def set(shader: BaseShader, inputID: Int, renderable: Renderable, combinedAttributes: Attributes): Unit = {
      val unit = shader.context.get.textureBinder.bind(
        combinedAttributes.getAs[TextureAttribute](PBRTextureAttribute.OcclusionTexture).get.textureDescription)
      PBRShaderUtils.setInt(shader, inputID, unit)
    }
  }
  ...
  val emissiveScaledColor: BaseShader.Setter = new BaseShader.LocalSetter {
    override def set(shader: BaseShader, inputID: Int, renderable: Renderable, combinedAttributes: Attributes): Unit = {
      ...
      if (eiOpt.isDefined) {
        val ei = eiOpt.get
        PBRShaderUtils.setFloat4(shader, inputID,
  ...
  val transmissionTextureSetter: BaseShader.Setter = new BaseShader.LocalSetter {
    override def set(shader: BaseShader, inputID: Int, renderable: Renderable, combinedAttributes: Attributes): Unit = {
      val unit = shader.context.get.textureBinder.bind(
        combinedAttributes.getAs[TextureAttribute](PBRTextureAttribute.TransmissionTexture).get.textureDescription)
      PBRShaderUtils.setInt(shader, inputID, unit)
    }
  }
  ...
  val thicknessTextureSetter: BaseShader.Setter = new BaseShader.LocalSetter {
    override def set(shader: BaseShader, inputID: Int, renderable: Renderable, combinedAttributes: Attributes): Unit = {
      val unit = shader.context.get.textureBinder.bind(
        combinedAttributes.getAs[TextureAttribute](PBRTextureAttribute.ThicknessTexture).get.textureDescription)
      PBRShaderUtils.setInt(shader, inputID, unit)
    }
  }
  ...
  val specularFactorTextureSetter: BaseShader.Setter = new BaseShader.LocalSetter {
    override def set(shader: BaseShader, inputID: Int, renderable: Renderable, combinedAttributes: Attributes): Unit = {
      val unit = shader.context.get.textureBinder.bind(
        combinedAttributes.getAs[TextureAttribute](PBRTextureAttribute.SpecularFactorTexture).get.textureDescription)
      PBRShaderUtils.setInt(shader, inputID, unit)
    }
  }
  ...
  val specularColorTextureSetter: BaseShader.Setter = new BaseShader.LocalSetter {
    override def set(shader: BaseShader, inputID: Int, renderable: Renderable, combinedAttributes: Attributes): Unit = {
      val unit = shader.context.get.textureBinder.bind(
        combinedAttributes.getAs[TextureAttribute](PBRTextureAttribute.SpecularColorTexture).get.textureDescription)
      PBRShaderUtils.setInt(shader, inputID, unit)
    }
  }
  ...
  val iridescenceTextureSetter: BaseShader.Setter = new BaseShader.LocalSetter {
    override def set(shader: BaseShader, inputID: Int, renderable: Renderable, combinedAttributes: Attributes): Unit = {
      val unit = shader.context.get.textureBinder.bind(
        combinedAttributes.getAs[TextureAttribute](PBRTextureAttribute.IridescenceTexture).get.textureDescription)
      PBRShaderUtils.setInt(shader, inputID, unit)
    }
  }
  ...
  val iridescenceThicknessTextureSetter: BaseShader.Setter = new BaseShader.LocalSetter {
    override def set(shader: BaseShader, inputID: Int, renderable: Renderable, combinedAttributes: Attributes): Unit = {
      val unit = shader.context.get.textureBinder.bind(
        combinedAttributes.getAs[TextureAttribute](PBRTextureAttribute.IridescenceThicknessTexture).get.textureDescription)
      PBRShaderUtils.setInt(shader, inputID, unit)
    }
  }
  ...
  val transmissionSourceTextureSetter: BaseShader.Setter = new BaseShader.LocalSetter {
    override def set(shader: BaseShader, inputID: Int, renderable: Renderable, combinedAttributes: Attributes): Unit = {
      val unit = shader.context.get.textureBinder.bind(
        combinedAttributes.getAs[TextureAttribute](PBRTextureAttribute.TransmissionSourceTexture).get.textureDescription)
      PBRShaderUtils.setInt(shader, inputID, unit)
    }
  }
  ...
  val transmissionSourceMipmapSetter: BaseShader.Setter = new BaseShader.LocalSetter {
    override def set(shader: BaseShader, inputID: Int, renderable: Renderable, combinedAttributes: Attributes): Unit = {
      val mipmapFactor = combinedAttributes.getAs[PBRTextureAttribute](PBRTextureAttribute.TransmissionSourceTexture).map { a =>
  ...
  val specularMirrorTextureSetter: BaseShader.Setter = new BaseShader.LocalSetter {
    override def set(shader: BaseShader, inputID: Int, renderable: Renderable, combinedAttributes: Attributes): Unit = {
      val unit = shader.context.get.textureBinder.bind(
        combinedAttributes.getAs[MirrorSourceAttribute](MirrorSourceAttribute.Type).get.textureDescription)
      PBRShaderUtils.setInt(shader, inputID, unit)
    }
  }
  ...
  val specularMirrorMipmapSetter: BaseShader.Setter = new BaseShader.LocalSetter {
    override def set(shader: BaseShader, inputID: Int, renderable: Renderable, combinedAttributes: Attributes): Unit = {
      val mipmapFactor = combinedAttributes.getAs[MirrorSourceAttribute](MirrorSourceAttribute.Type).map { a =>
        (Math.log(a.textureDescription.texture.get.width.toDouble) / Math.log(2.0)).toFloat
      }.getOrElse(1f)
      PBRShaderUtils.setFloat(shader, inputID, mipmapFactor)
    }
