...
/** Contains code for manipulating colors as `int` and packed `float` values in the IPT color space. IPT has more perceptually-uniform handling of hue than some other color spaces.
  */
object ColorTools {
  ...
  /** Converts a packed float IPT color to an RGBA8888 int. */
  ...
  /** Converts a packed float IPT color to a packed float in RGBA format. */
  ...
  /** Writes an IPT packed float color into an RGBA8888 Color. */
  ...
  /** Takes an RGBA8888 int and converts to a packed float in IPT format. */
  ...
  /** Takes a packed float RGBA color and converts to a packed float in IPT format. */
  ...
  /** Takes a Color (RGBA8888) and converts to a packed float in IPT format. */
  ...
  /** Takes RGBA components from 0.0 to 1.0 and converts to a packed float in IPT format. */
  ...
  /** The "intensity" of the given packed float in IPT format (lightness, 0 to 1). */
  def intensity(encoded: Float): Float =
    (java.lang.Float.floatToRawIntBits(encoded) & 0xff) / 255f

  /** The "protan" of the given packed float in IPT format (0 to 1). */
  def protan(encoded: Float): Float =
    (java.lang.Float.floatToRawIntBits(encoded) >>> 8 & 0xff) / 255f

  /** The "tritan" of the given packed float in IPT format (0 to 1). */
  ...
  /** Gets a color as an IPT packed float given floats for HSL and opacity. */
  ...
  /** Checks whether the given IPT color (I, P, T, alpha) is in-gamut; if not, brings it inside. */
  def limitToGamut(I: Float, P: Float, T: Float, alpha: Float): Float =
    ipt(Math.min(Math.max(I, 0f), 1f), Math.min(Math.max(P, 0f), 1f), Math.min(Math.max(T, 0f), 1f), Math.min(Math.max(alpha, 0f), 1f))

  /** Checks whether the given IPT packed float is in-gamut; if not, brings it inside. */
  def limitToGamut(packed: Float): Float =
    packed

  /** Makes the additive IPT color cause less of a change when used as a tint. */
