 ...
 * Original source: com/github/tommyettinger/colorful/rgb/TextureArrayColorfulBatch.java
...
class TextureArrayColorfulBatch(size: Int = 1000, defaultShader: Nullable[ShaderProgram] = Nullable.empty)(using Sge) extends Batch with AutoCloseable {
  ...
  /** Sets the color to the result of {@link ColorTools#rgb(float, float, float, float)} on the same arguments. For the RGB parameters, 0.5f is a neutral value (causing no change), while for alpha,
    ...
    * @param r
    *   red; additive; ranges from 0 (remove red) to 1 (add red)
    * @param g
    *   green; additive; ranges from 0 (remove green) to 1 (add green)
    * @param b
    *   blue; additive; ranges from 0 (remove blue) to 1 (add blue)
    * @param a
  ...
  /** Sets the color with the given R/G/B and A parameters from 0 to 255. For the R/G/B parameters, 127 or 128 is a neutral value (causing no change), while for alpha, 255 is a neutral value. For
    * R/G/B, higher values will add to the corresponding channel, while lower values will subtract from it.
    * @see
    *   ColorTools#rgb(float, float, float, float)
    * @param r
    *   red; additive; ranges from 0 (decrease) to 255 (increase)
    * @param g
    *   green; additive; ranges from 0 (decrease) to 255 (increase)
    * @param b
    *   blue; additive; ranges from 0 (decrease) to 255 (increase)
    * @param alpha
    *   opacity, from 0 to 255 (254 is equivalent, since the lowest bit is discarded); multiplicative
    */
  def setIntColor(r: Int, g: Int, b: Int, alpha: Int): Unit =
    colorPacked = NumberUtils.intBitsToFloat(
      (alpha << 24 & 0xfe000000)
        | (b << 16 & 0xff0000) | (g << 8 & 0xff00) | (r & 0xff)
    )

  /** Takes the tweak as an int in the format: R (8 bits), G (8 bits), B (8 bits), contrast (7 bits), (1 ignored bit at the end).
    * @param tweak
    *   the tweak to use as an integer, with R in the most significant bits and contrast in least
    */
  ...
  def setTweakedColor(
    ...
    setColor(addR, addG, addB, mulAlpha)
    setTweak(mulR, mulG, mulB, contrast)
  }
...
object TextureArrayColorfulBatch {
  ...
  /** Returns a new instance of the default shader used by TextureArrayColorfulBatch for GL2 when no shader is specified. Does not have any {@code #version} specified in the shader source. This
    ...
    *   look this up once with {@link #getMaxTextureUnits()} for the current hardware
    * @param vertex
    *   typically {@link #vertexShader}, but can also be user-defined
    * @param fragment
    *   typically {@link #fragmentShader}, but can also be {@link #fragmentShaderAlternateTintCenter} or user-defined
    * @return
    *   the default ShaderProgram for this Batch
    */
  ...
  /** Creates a ShaderProgram that can be passed to {@link TextureArrayColorfulBatch}; it will accept RGBA colors in {@link TextureArrayColorfulBatch#setColor(float, float, float, float)}, but
    * {@link TextureArrayColorfulBatch#setTweak(float, float, float, float)} will treat its inputs as luma, chroma A, chroma B, and contrast, as Oklab handles them. This can be more useful than an
    * RGBA tweak because this allows you to make images grayscale (by setting chroma A and chroma B to 0.0) or hyper-saturated (by setting those same two to values above 0.5), and you can also
    * colorize an image by making it grayscale and then adding a non-gray color with {@link TextureArrayColorfulBatch#color_=}. Changing the tweak's luma can act similarly to changing the contrast.
    * Changing just one of chroma A or B can have interesting effects, especially if you want to simulate red-green color-blindness (protanopia) by setting chroma A to 0, or another kind of
    * color-blindness (tritanopia) by setting chroma B to 0.
    *
    * @return
    *   a new ShaderProgram that takes an RGBA tint color and an Oklab tweak
    */
  def createShaderWithOklabTweak()(using Sge): ShaderProgram =
    TextureArrayColorfulBatch.createDefaultShader(
      TextureArrayColorfulBatch.getMaxTextureUnits(),
      sge.colorful.oklab.TextureArrayColorfulBatch.vertexShaderOklabWithRGBATint,
      sge.colorful.oklab.TextureArrayColorfulBatch.fragmentShader
    )

  /** The default shader's vertex part. <br> This is meant to be used with {@link #fragmentShader} or {@link #fragmentShaderAlternateTintCenter}.
    */
  ...
  /** The default shader's fragment part. <br> This must have the String <code>@maxTextureUnits@</code> replaced with the value of {@link #_maxTextureUnits} at runtime. This is done by
    * {@link #createDefaultShader(int, String, String)}, but not if you create a {@link ShaderProgram} yourself. <br> This is meant to be used with {@link #vertexShader} and passed to
    * {@link TextureArrayColorfulBatch}
    */
