 ...
 * Original source: colorful/src/main/java/com/github/tommyettinger/colorful/oklab/ColorfulSprite.java
 ...
 * Covenant-source-reference: colorful/src/main/java/com/github/tommyettinger/colorful/oklab/ColorfulSprite.java
...
import sge.colorful.oklab.ColorfulBatch.*
...
class ColorfulSprite() extends TextureRegion {
  ...
  /** Sets the color used to tint this sprite. Default is {@link Palette#GRAY} , which makes no changes to the color. Use {@link ColorTools#oklab(float, float, float, float)} or a predefined color
  ...
  /** Sets the color used to tint this sprite and the tweak that affects how that color will be treated. Default color is {@link Palette#GRAY} , which makes no changes to the color, and default tweak
    * is {@link ColorfulBatch#TWEAK_RESET} , which resets any changes to the tweak back to a neutral state. You can easily get a tweak value with {@link ColorTools#oklab(float, float, float, float)} ,
  ...
  def setTweakedColor(addL: Float, addA: Float, addB: Float, mulAlpha: Float, mulL: Float, mulA: Float, mulB: Float, contrast: Float): Unit = {
    val color    = ColorTools.oklab(addL, addA, addB, mulAlpha)
    val tweak    = ColorTools.oklab(mulL, mulA, mulB, contrast)
    val vertices = this._vertices
    vertices(C1) = color
    vertices(C2) = color
    vertices(C3) = color
  ...
  /** Sets the tweak that affects how the rendered color will be treated. Default tweak is {@link ColorfulBatch#TWEAK_RESET} , which resets any changes to the tweak back to a neutral state. You can
    * easily get a tweak value with {@link ColorTools#oklab(float, float, float, float)} , just using the last parameter to represent contrast.
    *
    * @param tweak
    *   the packed float used to multiply the L/A/B channels, as well as the setting for contrast
    */
  ...
  /** Given an RGBA8888 Color from libGDX, this converts that to an Oklab color as a packed float and sets the color of the sprite using that.
    *
    * @param color
  ...
  def setColor(L: Float, A: Float, B: Float, alpha: Float): Unit = {
    val color    = ColorTools.oklab(L, A, B, alpha)
    val vertices = this._vertices
    vertices(C1) = color
    vertices(C2) = color
    vertices(C3) = color
  ...
  def setTweak(L: Float, A: Float, B: Float, contrast: Float): Unit = {
    val tweak    = ColorTools.oklab(L, A, B, contrast)
    val vertices = this._vertices
    vertices(T1) = tweak
    vertices(T2) = tweak
    vertices(T3) = tweak
