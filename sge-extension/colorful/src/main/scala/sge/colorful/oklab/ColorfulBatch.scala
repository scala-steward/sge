 ...
 * Original source: com/github/tommyettinger/colorful/oklab/ColorfulBatch.java
...
/** A substitute for {@link sge.graphics.g2d.SpriteBatch} that adds an additional attribute to store an extra color's worth of channels, used to modify the L, A, and B channels of a color by
  ...
  * ColorfulBatch uses the Oklab color space where SpriteBatch normally uses RGBA, which means that in the batch color, red maps to additive L (lightness), green maps to additive A (green-to-red),
  * blue maps to additive B (violet-to-yellow), and alpha continues to be multiplicative alpha. Additive values cause no change when they are 0.5, but cause a sharp increase at 1.0 and a sharp
  * decrease at 0.0. In the tweak, red maps to multiplicative L, green maps to multiplicative A, blue maps to multiplicative B, and alpha maps to "exponential-like" contrast. Like with the additive
  * values, 0.5 causes no change for multiplicative values, but you can think of the existing color as being temporarily shifted down by 0.5 for each of the LAB channels, so each LAB channel's range
  * is -0.5 to 0.5 before being multiplied by the corresponding tweak value (times 2.0, which means multiplicative tweak values for A and B that are less than 0.5 make colors closer to grayscale and
  * values greater than 0.5 make colors more vibrant) and then shifted back to its prior range. Contrast is a special value; 0.5 still means "no change," but lower values will make L changes less
  * distinct between similar colors, while higher values will "sharpen" the changes in L. <br> Created by Tommy Ettinger on 1/21/2021.
  *
  * @param size
...
object ColorfulBatch {
  ...
  /** A special-purpose vertex shader meant for use only here in the Oklab ColorfulBatch, this can be used to create a ShaderProgram and passed into the constructor or {@link ColorfulBatch#shader_=}.
    * Using this vertex shader lets you specify the batch color (or the tint) as a "normal" RGBA color, while using Oklab for the tweak. The fragment shader doesn't need to be modified here, so you
    * can use {@link #fragmentShader} as normal.
    */
