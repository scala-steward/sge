...
/** Static methods for handling gradients of smoothly-changing colors, typically inside of ArrayBuffers. The intent is for the ArrayBuffer to be used as a sequence of packed float Oklab colors. This
  * class does some special handling for Oklab colors, limiting to the correct gamut.
  */
object GradientTools {

  /** Creates an ArrayBuffer gradient from the packed float Oklab color `start` to `end`, taking the specified number of steps and using linear interpolation.
    */
  ...
  /** Appends a gradient between several packed float Oklab colors provided in `chain`.
    */
