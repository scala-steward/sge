...
/** Contains code for manipulating colors as `int` and packed `float` values in the YCwCm color space. IPT has more color manipulation in the YCwCm color space.
  */
object ColorTools {
  ...
  /** Converts a packed float YCwCm color to an RGBA8888 int. */
  ...
  /** Converts a packed float YCwCm color to a packed float in RGBA format. */
  ...
  /** Writes an YCwCm packed float color into an RGBA8888 Color. */
  ...
  /** Takes an RGBA8888 int and converts to a packed float in YCwCm format. */
  ...
  /** Takes a packed float RGBA color and converts to a packed float in YCwCm format. */
  ...
  /** Takes a Color (RGBA8888) and converts to a packed float in YCwCm format. */
  ...
  /** Takes RGBA components from 0.0 to 1.0 and converts to a packed float in YCwCm format. */
  ...
  /** The "luma" of the given packed float in YCwCm format (lightness, 0 to 1). */
  def luma(encoded: Float): Float =
    (java.lang.Float.floatToRawIntBits(encoded) & 0xff) / 255f

  /** The "warm" of the given packed float in YCwCm format (0 to 1). */
  def warm(encoded: Float): Float =
    (java.lang.Float.floatToRawIntBits(encoded) >>> 8 & 0xff) / 255f

  /** The "mild" of the given packed float in YCwCm format (0 to 1). */
  def mild(encoded: Float): Float =
  ...
  def dullen(start: Float, change: Float): Float = {
    ...
    ycwcm((s & 0xff) / 255f, ((s >>> 8 & 0xff) / 255f - 0.5f) * (1f - change) + 0.5f, ((s >>> 16 & 0xff) / 255f - 0.5f) * (1f - change) + 0.5f, (s >>> 25) / 127f)
  }
  ...
  /** Gets a color as an YCwCm packed float given floats for HSL and opacity. */
  ...
  /** Checks whether the given YCwCm color (I, P, T, alpha) is in-gamut; if not, brings it inside. */
  def limitToGamut(I: Float, P: Float, T: Float, alpha: Float): Float =
    ycwcm(Math.min(Math.max(I, 0f), 1f), Math.min(Math.max(P, 0f), 1f), Math.min(Math.max(T, 0f), 1f), Math.min(Math.max(alpha, 0f), 1f))

  /** Checks whether the given YCwCm packed float is in-gamut; if not, brings it inside. */
  def limitToGamut(packed: Float): Float =
    packed

  /** Makes the additive YCwCm color cause less of a change when used as a tint. */
