 ...
 * Original source: com/github/tommyettinger/colorful/ycwcm/ColorfulBatch.java
 ...
 * Covenant-source-reference: com/github/tommyettinger/colorful/ycwcm/ColorfulBatch.java
...
/** A substitute for {@link sge.graphics.g2d.SpriteBatch} that adds an additional attribute to store an extra color's worth of channels, used to modify the luma and chromatic channels of a color by
  ...
  * ColorfulBatch uses the YCwCm color space where SpriteBatch normally uses RGBA, which means that in the batch color, red maps to additive Y (luma/lightness), green maps to additive Cw (chromatic
  * warmth), blue maps to additive Cm (chromatic mildness), and alpha continues to be multiplicative alpha. Additive values cause no change when they are 0.5, but cause a sharp increase at 1.0 and a
  * sharp decrease at 0.0. In the tweak, red maps to multiplicative Y, green maps to multiplicative Cw, blue maps to multiplicative Cm, and alpha maps to "exponential-like" contrast. Like with the
  * additive values, 0.5 causes no change for multiplicative values, and the tweak modifies the base color channels by multiplication. Contrast is a special value; 0.5 still means "no change," but
  * lower values will make Y changes less distinct between similar colors, while higher values will "sharpen" the changes in Y. <br> Created by Tommy Ettinger on 12/14/2019.
  *
  * @param size
...
class ColorfulBatch(size: Int = 1000, defaultShader: Nullable[ShaderProgram] = Nullable.empty)(using Sge) extends Batch with AutoCloseable {
  ...
  /** A packed float color multiplied with 2.0 and the base color of a drawn pixel (which is typically from a Texture). This can be created with {@link ColorTools#ycwcm(float, float, float, float)}
    * like any other packed float color, but the YCwCm channels instead refer to multiplicative Y, Cw, and Cm, and alpha used instead for contrast. Giving this a channel value of 0.5 has nearly no
    * change and 0.0 or 1.0 has extreme changes.
    */
  ...
  /** Sets the additive changes to the YCwCm color, as well as the multiplicative change to the alpha.
    * @param r
    *   additive luma channel, from 0 to 1; 0.5 is neutral
    * @param g
    *   additive chromatic warmth channel, from 0 to 1; 0.5 is neutral
    * @param b
    *   additive chromatic mildness channel, from 0 to 1; 0.5 is neutral
    * @param a
    *   multiplicative alpha channel, from 0 to 1; 1.0 is neutral
    */
  override def setColor(r: Float, g: Float, b: Float, a: Float): Unit =
    colorPacked = ColorTools.ycwcm(r, g, b, a)
  ...
  /** Sets the multiplicative changes to the YCwCm color, along with the contrast.
    * @param Y
    *   multiplicative luma channel, from 0 to 1; 0.5 is neutral
    * @param Cw
    *   multiplicative chromatic warmth channel, from 0 to 1; 0.5 is neutral
    * @param Cm
    *   multiplicative chromatic mildness channel, from 0 to 1; 0.5 is neutral
    * @param contrast
  ...
  def setTweak(Y: Float, Cw: Float, Cm: Float, contrast: Float): Unit =
    _tweak = ColorTools.ycwcm(Y, Cw, Cm, contrast)

  /** Sets the tweak using a single packed YCwCm float.
    * @see
    *   #setTweak(float, float, float, float)
    * @param tweak
    *   a packed YCwCm float, with contrast instead of alpha
    */
  ...
  /** Expects an int color in the format (lightness, 8 bits), (chroma warm, 8 bits), (chroma mild, 8 bits), (alpha, 7 bits), (ignored, 1 bit).
    * @param color
    *   an int color with alpha in the least significant byte and lightness in the most significant
    */
  def setIntColor(color: Int): Unit =
    this.colorPacked = NumberUtils.intBitsToFloat(Integer.reverseBytes(color & -2))

  /** Sets the color with the given Y/Cw/Cm and alpha parameters from 0 to 255. For the Y/Cw/Cm parameters, 127 or 128 is a neutral value (causing no change), while for alpha, 255 is a neutral value.
    * For Y/Cw/Cm, higher values will add to the corresponding channel, while lower values will subtract from it.
    * @see
    *   ColorTools#ycwcm(float, float, float, float)
    * @param luma
    *   additive luma; ranges from 0 (black) to 255 (white)
    * @param warm
    *   additive chromatic warmth; ranges from 0 to 255
    * @param mild
    *   additive chromatic mildness; ranges from 0 to 255
    * @param alpha
    *   opacity, from 0 to 255 (254 is equivalent, since the lowest bit is discarded); multiplicative
    */
  def setIntColor(luma: Int, warm: Int, mild: Int, alpha: Int): Unit =
    colorPacked = NumberUtils.intBitsToFloat(
      (alpha << 24 & 0xfe000000)
        | (mild << 16 & 0xff0000) | (warm << 8 & 0xff00) | (luma & 0xff)
    )

  /** Takes the tweak as an int in the format: Y (8 bits), Cw (8 bits), Cm (8 bits), contrast (7 bits), (1 ignored bit at the end).
    * @param tweak
    *   the tweak to use as an integer, with Y in the most significant bits and contrast in least
    */
  ...
  /** Sets the multiplicative and contrast parts of the shader's color changes. 127 is a neutral value that should have minimal effect on the image; using {@code (127, 127, 127, 127)} will effectively
    * remove the tweak.
    * @param luma
    *   multiplicative luma; ranges from 0 (black) to 255 (white)
    * @param warm
    *   multiplicative chromatic warmth; ranges from 0 to 255
    * @param mild
    *   multiplicative chromatic mildness; ranges from 0 to 255
    * @param contrast
    *   affects how lightness changes; ranges from 0 (low contrast, cloudy look) to 255 (high contrast, sharpened look)
    */
  def setIntTweak(luma: Int, warm: Int, mild: Int, contrast: Int): Unit =
  ...
  def setTweakedColor(
    ...
    setColor(lumaAdd, warmAdd, mildAdd, mulAlpha)
