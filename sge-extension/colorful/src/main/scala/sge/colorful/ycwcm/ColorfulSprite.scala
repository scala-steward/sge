 ...
 * Original source: colorful/src/main/java/com/github/tommyettinger/colorful/ycwcm/ColorfulSprite.java
 ...
 * Covenant-source-reference: colorful/src/main/java/com/github/tommyettinger/colorful/ycwcm/ColorfulSprite.java
...
import sge.colorful.ycwcm.ColorfulBatch.*
...
class ColorfulSprite() extends TextureRegion {
  ...
  /** Sets the color used to tint this sprite. Default is {@link Palette#GRAY} , which makes no changes to the color. Use {@link ColorTools#ycwcm(float, float, float, float)} or a predefined color
    * from {@link Palette} if you don't have a color currently.
    *
    * @param color
    *   the packed float color used to add luma, warmth, and mildness to the current sprite, as well as the multiplier for alpha
    */
  ...
  /** Sets the color used to tint this sprite and the tweak that affects how that color will be treated.
    ...
    *   the packed float color used to add luma, warmth, and mildness to the current sprite, as well as the multiplier for alpha
    * @param tweak
    *   the packed float used to multiply luma, warmth and mildness, as well as the setting for contrast
    */
  ...
  /** Sets the color used to tint this sprite and the tweak that affects how that color will be treated.
    ...
    * @param warmAdd
    *   how much warmth to add; coolest is 0f, neutral is 0.5f, warmest is 1f
    * @param mildAdd
    *   how much mildness to add; boldest is 0f, neutral is 0.5f, mildest is 1f
    * @param alphaMul
    ...
    * @param warmMul
    *   how much source warmth should be multiplied by; no cool/warm is 0f, neutral is 0.5f, max cool/warm is 1f
    * @param mildMul
    *   how much source mildness should be multiplied by; no bold/mild is 0f, neutral is 0.5f, max bold/mild is 1f
    * @param contrast
    ...
    */
  def setTweakedColor(lumaAdd: Float, warmAdd: Float, mildAdd: Float, alphaMul: Float, lumaMul: Float, warmMul: Float, mildMul: Float, contrast: Float): Unit = {
    val color    = ColorTools.ycwcm(lumaAdd, warmAdd, mildAdd, alphaMul)
    val tweak    = ColorTools.ycwcm(lumaMul, warmMul, mildMul, contrast)
    val vertices = this._vertices
    vertices(C1) = color
    vertices(C2) = color
    vertices(C3) = color
  ...
  /** Given an RGBA8888 Color from libGDX, this converts that to a YCwCm color as a packed float and sets the color of the sprite using that.
    *
    * @param color
  ...
  /** @see #setColor(float) */
  def setColor(luma: Float, warm: Float, mild: Float, alpha: Float): Unit = {
    val color    = ColorTools.ycwcm(luma, warm, mild, alpha)
    val vertices = this._vertices
    vertices(C1) = color
    vertices(C2) = color
    vertices(C3) = color
  ...
  /** @see #setTweak(float) */
  def setTweak(luma: Float, warm: Float, mild: Float, contrast: Float): Unit = {
    val tweak    = ColorTools.ycwcm(luma, warm, mild, contrast)
    val vertices = this._vertices
    vertices(T1) = tweak
    vertices(T2) = tweak
    vertices(T3) = tweak
  ...
  /** Returns the color of this sprite. If the returned instance is manipulated, {@link #setColor(float)} must be called afterward.
    *
    * @return
    *   a packed float color that stores luma addend, warmth addend, mildness addend, and alpha multiplier
    */
  def color: Float =
    _vertices(C1)
  ...
  /** Returns the multiplicative color tweaks used by this sprite, as a packed float with the same format as a color.
    *
    * @return
    *   a packed float that stores luma multiplier, warmth multiplier, mildness multiplier, and contrast
    */
