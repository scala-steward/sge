 ...
 * Original source: com/github/tommyettinger/colorful/ipt_hq/ColorfulBatch.java
...
/** A substitute for {@link sge.graphics.g2d.SpriteBatch} that adds an additional attribute to store an extra color's worth of channels, used to modify the intensity and chromatic channels of a color
  ...
  * ColorfulBatch uses the IPT (HQ) color space where SpriteBatch normally uses RGBA, which means that in the batch color, red maps to additive intensity, green maps to additive protan, blue maps to
  ...
  * while higher values will "sharpen" the changes in intensity.
  *
  * This is different from the {@link sge.colorful.ipt.ColorfulBatch} class in the ipt package; this class gamma-corrects colors (with a gamma of 2.0 for simplicity's sake) and uses a power of 0.43 or
  * 1.0/0.43 for some operations internally whereas the non-HQ version doesn't use any exponentiation. The HQ is short for High Quality. <br> Created by Tommy Ettinger on 12/14/2019.
  *
  * @param size
...
class ColorfulBatch(size: Int = 1000, defaultShader: Nullable[ShaderProgram] = Nullable.empty)(using Sge) extends Batch with AutoCloseable {
  ...
  def setTweak(I: Float, P: Float, T: Float, contrast: Float): Unit =
    _tweak = ColorTools.ipt(I, P, T, contrast)
  ...
  /** Sets the color with the given IPT and A parameters from 0 to 255. For the IPT parameters, 127 or 128 is a neutral value (causing no change), while for alpha, 255 is a neutral value. For IPT,
    ...
    */
  def setIntColor(I: Int, P: Int, T: Int, alpha: Int): Unit =
    colorPacked = NumberUtils.intBitsToFloat(
      (alpha << 24 & 0xfe000000)
        | (T << 16 & 0xff0000) | (P << 8 & 0xff00) | (I & 0xff)
    )
  ...
  /** Sets the multiplicative and contrast parts of the shader's color changes. 127 is a neutral value that should have minimal effect on the image; using {@code (127, 127, 127, 127)} will effectively
    ...
    */
  def setIntTweak(I: Int, P: Int, T: Int, contrast: Int): Unit =
  ...
  /** A convenience method that sets both the color (with {@link #setColor(float, float, float, float)}) and the tweak (with {@link #setTweak(float, float, float, float)}) at the same time.
    ...
    *   opacity, from 0 to 1; multiplicative
    * @param mulI
    *   like lightness; multiplicative; ranges from 0 (sets all I to 0) to 1 (doubles the image's I)
    * @param mulP
  ...
  def setTweakedColor(
    ...
    setColor(addI, addP, addT, mulAlpha)
    setTweak(mulI, mulP, mulT, contrast)
  }
