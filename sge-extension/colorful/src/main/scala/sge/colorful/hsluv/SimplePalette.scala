 ...
 * Covenant-baseline-methods: ALIASES,APRICOT,BLACK,BLUE,BRICK,BRONZE,BROWN,BUTTER,CACTUS,CELERY,CHARTREUSE,CHOCOLATE,CINNAMON,COBALT,COLORS_BY_HUE,CYAN,DENIM,EMBER,FERN,GRAY,GREEN,INDIGO,JADE,LAVENDER,LIME,LIST,MAGENTA,MAUVE,MINT,MOSS,NAMED,NAMES,NAMES_BY_HUE,NAMES_BY_LIGHTNESS,NAVY,OLIVE,ORANGE,PEACH,PEAR,PINK,PLUM,PURPLE,RASPBERRY,RED,ROSE,SAFFRON,SAGE,SALMON,SILVER,SKY,SimplePalette,TAN,TEAL,TRANSPARENT,TURQUOISE,VIOLET,WHITE,YELLOW,appendToKnownColors,bestCode,bestDistance,bestMatch,c,colorTries,colors,colorsByHue,combinedAdjectives,description,e,editKnownColors,end,i,idx,lightAdjectives,lightness,mc,mixing,mixingArr,namesByHue,paletteSize,parseDescription,satAdjectives,saturation,targetH,targetL,targetS,terms,totalTries,trn
 * Covenant-source-reference: com/github/tommyettinger/colorful/hsluv/SimplePalette.java
...
object SimplePalette {
  ...
  locally {
    ...
    NAMES_BY_HUE.sortInPlace()(using
      Ordering.fromLessThan[String] { (o1, o2) =>
        ...
        val s1  = ColorTools.channelS(c1)
        val s2  = ColorTools.channelS(c2)
        val cmp =
          if (c1 >= 0f) -10000
          else if (c2 >= 0f) 10000
            ...
            2 * Math.signum(ColorTools.channelH(c1) - ColorTools.channelH(c2)).toInt +
              Math.signum(ColorTools.channelL(c1) - ColorTools.channelL(c2)).toInt
        cmp < 0
      }
    )
  ...
  /** Given a color as a packed HSLuv float, this finds the closest description it can to match the given color while using at most `mixCount` colors to mix in. You should only use small numbers for
    ...
    * @param hsluv
    *   a packed HSLuv float color to attempt to match
    * @param mixCount
    ...
    */
  def bestMatch(hsluv: Float, mixCount: Int): String = {
    ...
    val targetH      = ColorTools.channelH(hsluv)
    val targetS      = ColorTools.channelS(hsluv)
    val targetL      = ColorTools.channelL(hsluv)
    ...
    while (c < totalTries) {
      ...
      var result = ColorTools.mix(mixingArr, 0, mc)
      ...
      var dH = Math.abs(ColorTools.channelH(result) - targetH)
      if (dH > 0.5f) dH = 1f - dH
      val dS       = ColorTools.channelS(result) - targetS
      val dL       = ColorTools.channelL(result) - targetL
      val distance = dH * dH + dS * dS + dL * dL
  ...
  /** Changes the existing RGBA Color instances in [[Colors]] to use HSLuv and so be able to be shown normally by ColorfulBatch. Any colors used in text markup look up their values in Colors, so
  ...
  /** Appends HSLuv-compatible Color instances to the map in [[Colors]], using the names in [[NAMES]] (which are "lower cased" instead of "ALL UPPER CASE"). This doesn't need any changes to be made to
  ...
  def appendToKnownColors(): Unit =
    for ((key, value) <- NAMED) {
      val f = value
      Colors.put(key, new Color(ColorTools.channelH(f), ColorTools.channelS(f), ColorTools.channelL(f), ColorTools.alpha(f)))
    }
}
