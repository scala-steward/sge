 ...
 * Original source: colorful/src/main/java/com/github/tommyettinger/colorful/hsluv/ColorfulSprite.java
 ...
 * Covenant-source-reference: colorful/src/main/java/com/github/tommyettinger/colorful/hsluv/ColorfulSprite.java
...
import sge.colorful.hsluv.ColorfulBatch.*
...
class ColorfulSprite() extends TextureRegion {
  ...
  /** Sets the color used to tint this sprite. Default is {@link Palette#GRAY} , which makes no changes to the color. Use {@link ColorTools#hsluv(float, float, float, float)} or a predefined color
    * from {@link Palette} if you don't have a color currently.
    *
    * @param color
    *   the packed float color used to add to the H/S/L channels of the current sprite, as well as the multiplier for alpha
    */
  ...
  /** Sets the color used to tint this sprite and the tweak that affects how that color will be treated. Default color is {@link Palette#GRAY} , which makes no changes to the color, and default tweak
    * is {@link ColorfulBatch#TWEAK_RESET} , which resets any changes to the tweak back to a neutral state. You can easily get a tweak value with {@link ColorTools#hsluv(float, float, float, float)} ,
    * just using the last parameter to represent contrast.
    *
    * @param color
    *   the packed float color used to add to the H/S/L channels of the current sprite, as well as the multiplier for alpha
    * @param tweak
    *   the packed float used to multiply the H/S/L channels, as well as the setting for contrast
    */
  ...
  /** Sets the color used to tint this sprite and the tweak that affects how that color will be treated. Default color is {@link Palette#GRAY} , which makes no changes to the color, and default tweak
    ...
    */
  def setTweakedColor(addH: Float, addS: Float, addL: Float, mulAlpha: Float, mulH: Float, mulS: Float, mulL: Float, contrast: Float): Unit = {
    val color    = ColorTools.hsluv(addH, addS, addL, mulAlpha)
    val tweak    = ColorTools.hsluv(mulH, mulS, mulL, contrast)
    val vertices = this._vertices
    vertices(C1) = color
    vertices(C2) = color
    vertices(C3) = color
  ...
  /** Sets the tweak that affects how the rendered color will be treated. Default tweak is {@link ColorfulBatch#TWEAK_RESET} , which resets any changes to the tweak back to a neutral state. You can
    * easily get a tweak value with {@link ColorTools#hsluv(float, float, float, float)} , just using the last parameter to represent contrast.
    *
    * @param tweak
    *   the packed float used to multiply the H/S/L channels, as well as the setting for contrast
    */
  ...
  /** Given an RGBA8888 Color from libGDX, this converts that to an HSLuv color as a packed float and sets the color of the sprite using that.
    *
    * @param color
  ...
  def setColor(h: Float, s: Float, l: Float, alpha: Float): Unit = {
    val color    = ColorTools.hsluv(h, s, l, alpha)
    val vertices = this._vertices
    vertices(C1) = color
    vertices(C2) = color
    vertices(C3) = color
  ...
  def setTweak(h: Float, s: Float, l: Float, contrast: Float): Unit = {
    val tweak    = ColorTools.hsluv(h, s, l, contrast)
    val vertices = this._vertices
    vertices(T1) = tweak
    vertices(T2) = tweak
    vertices(T3) = tweak
  ...
  /** Returns the color of this sprite. If the returned instance is manipulated, {@link #setColor(float)} must be called afterward.
    *
    * @return
    *   a packed float color used to add to the H/S/L channels of the current sprite, as well as the multiplier for alpha
    */
  def color: Float =
    _vertices(C1)
  ...
  /** Returns the multiplicative color tweaks used by this sprite, as a packed float with the same format as a color.
    *
    * @return
    *   a packed float used to multiply the H/S/L channels, as well as the setting for contrast
    */
