 ...
 * Covenant-source-reference: com/github/tommyettinger/colorful/hsluv/GradientTools.java
...
/** Static methods for handling gradients of smoothly-changing colors, typically inside of ArrayBuffers. The intent is for the ArrayBuffer to be used as a sequence of packed float HSLuv colors. This
  * class uses HSLuv-specific lerping that correctly handles hue wrapping.
  */
object GradientTools {

  /** Creates an ArrayBuffer gradient from the packed float HSLuv color `start` to `end`, taking the specified number of steps and using linear interpolation.
    */
  ...
  /** Appends a gradient between several packed float HSLuv colors provided in `chain`.
    */
  def appendGradientChain(appending: ArrayBuffer[Float], steps: Int, interpolation: Interpolation = Interpolation.linear, chain: Float*): ArrayBuffer[Float] =
    if (steps <= 0 || chain.isEmpty) {
      ...
      while (i < limit) {
        ...
        appending += ColorTools.lerpFloatColors(chain(idx), chain(idx + 1), MathUtils.norm(idx.toFloat, (idx + 1).toFloat, splint))
        change += step
        i += 1
      }
  ...
  def appendPartialGradient(appending: ArrayBuffer[Float], start: Float, end: Float, steps: Int, interpolation: Interpolation = Interpolation.linear): ArrayBuffer[Float] =
    if (steps <= 0) {
      ...
      var i      = 0
      while (i < steps) {
        appending += ColorTools.lerpFloatColors(start, end, interpolation.apply(change))
        change += step
        i += 1
      }
      appending
