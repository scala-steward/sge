 ...
 * Original source: com/github/tommyettinger/colorful/hsluv/ColorfulBatch.java
 ...
 * Covenant-source-reference: com/github/tommyettinger/colorful/hsluv/ColorfulBatch.java
...
/** A substitute for {@link sge.graphics.g2d.SpriteBatch} that adds an additional attribute to store an extra color's worth of channels, used to modify the H, S, and L channels of a color by
  ...
  * ColorfulBatch uses the HSLuv color space where SpriteBatch normally uses RGBA, which means that in the batch color, red maps to additive H (hue), green maps to additive S (saturation), blue maps
  * to additive L (lightness), and alpha continues to be multiplicative alpha. Additive values cause no change when they are 0.5, but cause a sharp increase at 1.0 and a sharp decrease at 0.0. In the
  * tweak, red maps to multiplicative H, green maps to multiplicative S, blue maps to multiplicative L, and alpha maps to "exponential-like" contrast. Like with the additive values, 0.5 causes no
  * change for multiplicative values, but you can think of the existing color as being temporarily shifted down by 0.5 for each of the HSL channels, so each HSL channel's range is -0.5 to 0.5 before
  * being multiplied by the corresponding tweak value (times 2.0, which means multiplicative tweak values for S that are less than 0.5 make colors closer to grayscale and values greater than 0.5 make
  * colors more vibrant) and then shifted back to its prior range. Contrast is a special value; 0.5 still means "no change," but lower values will make L changes less distinct between similar colors,
  * while higher values will "sharpen" the changes in L. The shader here is a port of
  * [[https://github.com/EsotericSoftware/hsl/blob/main/src/com/esotericsoftware/hsluv/Hsl.java this MIT-licensed code by Nathan Sweet]]. <br> Created by Tommy Ettinger on 1/21/2021.
  *
  * @param size
...
class ColorfulBatch(size: Int = 1000, defaultShader: Nullable[ShaderProgram] = Nullable.empty)(using Sge) extends Batch with AutoCloseable {
  ...
  /** A packed float color added to the base color of a drawn pixel (which is typically from a Texture) after the tweak has been multiplied with that base color.
    */
  protected var colorPacked: Float = ColorTools.hsluv(0f, 0f, 128f / 255f, 1f)

  /** Internal; not intended for external usage and undocumented. */
  private val tempColor: Color = Color(0f, 0f, 128f / 255f, 1f) // HSL from Palette.GRAY

  /** A packed float color multiplied with 2.0 and the base color of a drawn pixel (which is typically from a Texture). This can be created with {@link ColorTools#hsluv(float, float, float, float)}
    * like any other packed float color, but the HSL channels instead refer to multiplicative H, S, and L, and alpha used instead for contrast. This treats S as having been offset to match how HSLuv
    * normally represents saturation, centered on 0.0 instead of 0.5, before the multiplication. Giving this a channel value of 0.5 has nearly no change and 0.0 or 1.0 has extreme changes.
    */
  ...
  /** Sets the color to the result of {@link ColorTools#clamp(float, float, float, float)} on the same arguments. For the H/S/L parameters, 0.5f is a neutral value (causing no change), while for
    * alpha, 1.0f is a neutral value. For H/S/L, higher values will add to the corresponding channel, while lower values will subtract from it.
    * @see
    *   ColorTools#clamp(float, float, float, float)
    * @param r
    *   hue; additive; from 0 to 1, with red at 0, orange above red, yellow above orange, etc.
    * @param g
    *   saturation; additive; from 0 to 1, with 0 desaturating and 1 very saturating
    * @param b
    *   lightness; additive; ranges from 0 (closer to black) to 1 (closer to white)
    * @param a
    *   opacity, from 0 to 1; multiplicative
    */
  override def setColor(r: Float, g: Float, b: Float, a: Float): Unit =
    colorPacked = ColorTools.clamp(r, g, b, a)
  ...
  /** Sets the multiplicative and contrast parts of the shader's color changes. 0.5 is a neutral value that should have minimal effect on the image; using {@code (0.5f, 0.5f, 0.5f, 0.5f)} will
    ...
    * @param H
    *   hue; multiplicative; probably doesn't work very well and should just be 0.5f
    * @param S
    *   saturation; multiplicative; from 0 to 1, with 0 forcing grayscale and 1 emphasizing saturation
    * @param L
    *   lightness; multiplicative; ranges from 0 (sets lightness to 0) to 1 (doubles lightness)
    * @param contrast
  ...
  def setTweak(H: Float, S: Float, L: Float, contrast: Float): Unit =
    _tweak = ColorTools.clamp(H, S, L, contrast)

  /** Sets the tweak using a single packed HSLuv float.
    * @see
    *   #setTweak(float, float, float, float)
    * @param tweak
    *   a packed HSLuv float, with contrast instead of alpha
    */
  ...
  /** Expects an int color in the format (hue, 8 bits), (saturation, 8 bits), (lightness, 8 bits), (alpha, 7 bits), (ignored, 1 bit).
    * @param color
    *   an int color with alpha in the least significant byte and hue in the most significant
    */
  def setIntColor(color: Int): Unit =
    this.colorPacked = NumberUtils.intBitsToFloat(Integer.reverseBytes(color & -2))

  /** Sets the color with the given H/S/L and alpha parameters from 0 to 255. For the H/S/L parameters, 127 or 128 is a neutral value (causing no change), while for alpha, 255 is a neutral value. For
    * H/S/L, higher values will add to the corresponding channel, while lower values will subtract from it.
    * @see
    *   ColorTools#hsluv(float, float, float, float)
    * @param H
    *   hue; additive; from 0 to 255, with red at 0, orange above red, yellow above orange, etc.
    * @param S
    *   saturation; additive; from 0 to 255, with 0 desaturating and 255 very saturating
    * @param L
    *   lightness; additive; ranges from 0 (closer to black) to 255 (closer to white)
    * @param alpha
    *   opacity, from 0 to 255 (254 is equivalent, since the lowest bit is discarded); multiplicative
    */
  def setIntColor(H: Int, S: Int, L: Int, alpha: Int): Unit =
    colorPacked = NumberUtils.intBitsToFloat(
      (alpha << 24 & 0xfe000000)
        | (L << 16 & 0xff0000) | (S << 8 & 0xff00) | (H & 0xff)
    )

  /** Takes the tweak as an int in the format: H (8 bits), S (8 bits), L (8 bits), contrast (7 bits), (1 ignored bit at the end). An example would be 0x8020A06E, which doesn't change hue (using the
    * default 0x80), significantly reduces colorfulness with S multiplier 0x20 (closer to 0, so most colors will be almost grayscale), raises lightness somewhat with L multiplier 0xA0 (above the
    * neutral point of 0x80), and slightly reduces contrast (with contrast and the ignored bit as 0x6E, which is less than the halfway point).
    * @param tweak
    *   the tweak to use as an integer, with H in the most significant bits and contrast in least
    */
  def setIntTweak(tweak: Int): Unit =
    _tweak = NumberUtils.intBitsToFloat(Integer.reverseBytes(tweak & -2))

  /** Sets the multiplicative and contrast parts of the shader's color changes. 128 is a neutral value that should have minimal effect on the image; using {@code (128, 128, 128, 128)} will effectively
    * remove the tweak.
    * @param H
    *   hue; multiplicative; from 0 to 255, probably doesn't work very well and should just be 128
    * @param S
    *   saturation; multiplicative; from 0 to 255, with 0 forcing grayscale and 255 emphasizing saturation
    * @param L
    *   lightness; multiplicative; ranges from 0 (sets lightness to 0) to 255 (doubles lightness)
    * @param contrast
    *   affects how lightness changes; ranges from 0 (low contrast, cloudy look) to 255 (high contrast, sharpened look)
    */
  def setIntTweak(H: Int, S: Int, L: Int, contrast: Int): Unit =
