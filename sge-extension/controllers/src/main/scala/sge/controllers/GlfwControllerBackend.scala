...
/** Stub [[ControllerOps]] implementation documenting the GLFW functions needed for desktop controller support.
  *
  * When the sge-deps/native-components Rust module is extended to expose GLFW gamepad functions, this stub should be replaced with actual FFI calls. The required GLFW functions are:
  *
