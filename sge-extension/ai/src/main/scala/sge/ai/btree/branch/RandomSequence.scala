 ...
 * Original source: com/badlogic/gdx/ai/btree/branch/RandomSequence.java
 ...
 * Covenant-baseline-methods: RandomSequence,newInstance,start
 * Covenant-source-reference: com/badlogic/gdx/ai/btree/branch/RandomSequence.java
...
/** A `RandomSequence` is a sequence task's variant that runs its children in a random order.
  ...
  */
class RandomSequence[E](
  children: DynamicArray[Task[E]] = DynamicArray[Task[E]]()
) extends Sequence[E](children) {
  ...
  override def newInstance(): Task[E] = new RandomSequence[E]()
}
