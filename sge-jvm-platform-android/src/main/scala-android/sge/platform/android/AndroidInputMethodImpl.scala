    ...
    handler.post(new Runnable {
      override def run(): Unit = {
        ...
        if (validator != null) {
          filters += new InputFilter {
            override def filter(
              ...
              dest: _root_.android.text.Spanned, dstart: Int, dend: Int
            ): CharSequence = {
        ...
        if (autoComplete != null && autoComplete.length > 0) {
          editText.setAdapter(new ArrayAdapter[String](
            context,
      ...
      rootView match {
        case vg: _root_.android.view.ViewGroup =>
          val rootLp = new _root_.android.widget.FrameLayout.LayoutParams(
            _root_.android.widget.FrameLayout.LayoutParams.MATCH_PARENT,
            _root_.android.widget.FrameLayout.LayoutParams.MATCH_PARENT
          )
