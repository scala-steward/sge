...
class NumberUtilsTest extends munit.FunSuite {
  ...
  test("floatToRawIntBits preserves NaN payload".tag(munit.Tag("jvm-only"))) {
    // JS canonicalizes all NaN payloads to 0x7FC00000, so this test only works on JVM.
    // Create a specific NaN with a custom payload
    val isJs = try { Class.forName("scala.scalajs.js.Any"); true } catch { case _: Throwable => false }
    assume(!isJs, "NaN payload preservation is JVM-only")
