...
/** Detects mouse over, mouse or finger touch presses, and clicks on an actor. A touch must go down over the actor and is considered pressed as long as it is over the actor or within the
  ...
  *   Nathan Sweet
  */
class ClickListener(var button: Button = Button(0)) extends InputListener {
  ...
  private var _pressedButton:    Button  = Button(-1)
  ...
  override def touchDown(event: InputEvent, x: Float, y: Float, pointer: Int, button: Button): Boolean =
    if (_pressed) false
    else if (pointer == 0 && this.button != Button(-1) && button != this.button) false
    else {
      _pressed = true
  ...
  override def touchUp(event: InputEvent, x: Float, y: Float, pointer: Int, button: Button): Unit =
    if (pointer == _pressedPointer) {
      if (!cancelled) {
        ...
        if (touchUpOver && pointer == 0 && this.button != Button(-1) && button != this.button) touchUpOver = false
        if (touchUpOver) {
