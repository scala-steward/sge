...
/** A ScalingViewport that uses {@link Scaling#fill} so it keeps the aspect ratio by scaling the world up to take the whole screen (some of the world may be off screen).
  ...
  *   Nathan Sweet
  */
class FillViewport(worldWidth: WorldUnits, worldHeight: WorldUnits, camera: Camera)(using Sge) extends ScalingViewport(Scaling.fill, worldWidth, worldHeight, camera) {
