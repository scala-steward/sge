 ...
 *
 * upstream-commit: cc3678a655da064cb5cdff352e508db9f0609472
 */
package sge
package graphics
package glutils
...
class ShaderProgram(vertexShader: String, fragmentShader: String)(using Sge) extends AutoCloseable {
  ...
  private def compileShaders(vertexShader: String, fragmentShader: String): Unit = {
    vertexShaderHandle = loadShader(ShaderType.Vertex, vertexShader)
    fragmentShaderHandle = loadShader(ShaderType.Fragment, fragmentShader)
  ...
  /** Sets the vertex attribute with the given name. The {@link ShaderProgram} must be bound for this to work.
    ...
    */
  def setVertexAttribute(name: String, size: Int, shaderType: DataType, normalize: Boolean, stride: Int, buffer: Buffer): Unit = {
    val gl = Sge().graphics.gl20
    checkManaged()
  ...
  /** Sets the vertex attribute with the given name. The {@link ShaderProgram} must be bound for this to work.
    ...
    */
  def setVertexAttribute(name: String, size: Int, shaderType: DataType, normalize: Boolean, stride: Int, offset: Int): Unit = {
    val gl = Sge().graphics.gl20
    checkManaged()
    val location = fetchAttributeLocation(name)
    if (location != AttributeLocation.notFound) {
