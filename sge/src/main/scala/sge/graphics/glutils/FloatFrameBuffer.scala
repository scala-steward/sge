 ...
 * Covenant-baseline-methods: FloatFrameBuffer,bufferBuilder,checkExtensions,createTexture,data,result,this
 * Covenant-source-reference: com/badlogic/gdx/graphics/glutils/FloatFrameBuffer.java
...
class FloatFrameBuffer()(using Sge) extends FrameBuffer {
  ...
  def this(width: Int, height: Int, hasDepth: Boolean)(using Sge) = {
    this()
    checkExtensions()
    val bufferBuilder = GLFrameBuffer.FloatFrameBufferBuilder(Pixels(width), Pixels(height))
    bufferBuilder.addFloatAttachment(GL30.GL_RGBA32F, PixelFormat.RGBA, DataType.Float, false)
