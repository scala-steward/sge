 ...
 *   Convention: getSide() returns Nullable[CubemapSide] instead of raw null
 *   Convention: typed GL enums — TextureTarget for glFramebufferTexture2D textarget
 *   Audited: 2026-03-03
 *
 ...
 * Covenant-baseline-loc: 165
 * Covenant-baseline-methods: FrameBufferCubemap,attachFrameBufferColorTexture,bind,bindSide,createTexture,cubemapSides,currentSide,data,disposeColorTexture,frameBufferBuilder,gl,glHandle,nextSide,result,side,sides,this
 * Covenant-source-reference: com/badlogic/gdx/graphics/glutils/FrameBufferCubemap.java
...
class FrameBufferCubemap()(using Sge) extends GLFrameBuffer[Cubemap] {
  ...
  def this(format: Pixmap.Format, width: Int, height: Int, hasDepth: Boolean, hasStencil: Boolean)(using Sge) = {
    this()
    val frameBufferBuilder = GLFrameBuffer.FrameBufferCubemapBuilder(Pixels(width), Pixels(height))
  ...
  override protected def createTexture(attachmentSpec: GLFrameBuffer.FrameBufferTextureAttachmentSpec): Cubemap = {
    val data = GLOnlyTextureData(
      bufferBuilder.width.toInt,
      bufferBuilder.height.toInt,
      0,
      attachmentSpec.internalFormat,
      attachmentSpec.format,
      attachmentSpec.`type`
    )
    val result = Cubemap(data, data, data, data, data, data)
  ...
  protected def bindSide(side: Cubemap.CubemapSide): Unit =
    Sge().graphics.gl20.glFramebufferTexture2D(
      GL20.GL_FRAMEBUFFER,
