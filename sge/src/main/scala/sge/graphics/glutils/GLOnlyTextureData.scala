 ...
 *   Convention: typed GL enums — TextureTarget, PixelFormat, DataType for glTexImage2D
 *   Audited: 2026-03-03
 *
...
class GLOnlyTextureData(using Sge) extends TextureData {
  ...
  var format:         PixelFormat = scala.compiletime.uninitialized
  var `type`:         DataType    = scala.compiletime.uninitialized
  ...
  /** @see
    ...
    */
  def this(width: Int, height: Int, mipMapLevel: Int, internalFormat: Int, format: PixelFormat, `type`: DataType)(using Sge) = {
    this()
    this.width = width
    this.height = height
  ...
  override def consumeCustomData(target: TextureTarget): Unit =
    Sge().graphics.gl.glTexImage2D(target, mipLevel, internalFormat, Pixels(width), Pixels(height), 0, format, `type`, null)
