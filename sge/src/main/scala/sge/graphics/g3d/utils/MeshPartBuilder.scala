 ...
 *   - Deprecated shape methods deleted (no external users; use ShapeBuilder objects directly)
 *   - Convention: VertexInfo extends Pool.Poolable — given Poolable[VertexInfo] auto-derived via Poolable.fromTrait
 *   - Audit: pass (2026-03-03)
 *
 * Covenant: full-port
 * Covenant-baseline-spec-pass: 0
 * Covenant-baseline-loc: 361
 * Covenant-baseline-methods: MeshPartBuilder,VertexInfo,addMesh,attributes,color,ensureCapacity,ensureIndices,ensureRectangleIndices,ensureTriangleIndices,ensureVertices,getVertexTransform,hasColor,hasNormal,hasPosition,hasUV,index,isVertexTransformationEnabled,lastIndex,lerp,line,meshPart,normal,position,primitiveType,rect,reset,set,setCol,setColor,setNor,setPos,setUV,setUVRange,setVertexTransform,setVertexTransformationEnabled,triangle,uv,vertex
