...
import sge.graphics.{ Color, PrimitiveMode, VertexAttribute, VertexAttributes }
...
class ObjLoader(resolver: FileHandleResolver)(using Sge) extends ModelLoader[ObjLoader.ObjLoaderParameters](resolver) {
  ...
  private val groups: DynamicArray[ObjLoader.Group] = DynamicArray[ObjLoader.Group]()
  ...
  protected def loadModelData(file: FileHandle, flipV: Boolean): Nullable[ModelData] = boundary {
    ...
    while (g < numGroups) {
      ...
      part.primitiveType = PrimitiveMode.Triangles
      val mesh = ModelMesh()
      mesh.id = meshId
...
object ObjLoader {
  ...
  private class Group(val name: String) {
    ...
    var faces:        DynamicArray[Int] = DynamicArray[Int]()
    var numFaces:     Int               = 0
