...
class GL31Interceptor(glProfiler: GLProfiler, val gl31: GL31) extends GL30Interceptor(glProfiler, gl31) with GL31 {
  ...
  override def glTexStorage2DMultisample(target: TextureTarget, samples: Int, internalformat: Int, width: Int, height: Int, fixedsamplelocations: Boolean): Unit = {
    calls += 1
  ...
  override def glVertexAttribFormat(attribindex: Int, size: Int, `type`: DataType, normalized: Boolean, relativeoffset: Int): Unit = {
    calls += 1
