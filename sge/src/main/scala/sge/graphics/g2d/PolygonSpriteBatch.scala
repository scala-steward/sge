 ...
 *
 * Migration notes:
 *   Renames: dispose() -> close(); Java getters/setters -> Scala property-style accessors
 *   Convention: Nullable for shader; using Sge context parameter; AutoCloseable
 *   Idiom: boundary/break, Nullable, split packages
 *   Improvement: typed GL enums -- BlendFactor, BlendEquation, EnableCap -- see docs/improvements/opaque-types.md
 *   Audited: 2026-03-03
 *
...
class PolygonSpriteBatch(maxVertices: Int, maxTriangles: Int, defaultShader: Nullable[ShaderProgram])(using Sge) extends PolygonBatch {
  ...
  override def flush(): Unit =
    if (vertexIndex != 0) {
      ...
      mesh.render(customShader.getOrElse(_shader), PrimitiveMode.Triangles, 0, trianglesInBatch)
