...
import _root_.sge.graphics.{Color, Pixmap, Texture}
...
import _root_.sge.utils.{DynamicArray, Nullable, Pool, Poolable, ScreenUtils}
...
object ParticleShowGame extends DemoScene {
  ...
  override def render(dt: Float)(using Sge): Unit = {
    ...
    // Click/touch burst
    if (input.isTouched()) {
      val worldX = input.getX().toFloat / Sge().graphics.getWidth().toFloat * W
      val worldY = (1f - input.getY().toFloat / Sge().graphics.getHeight().toFloat) * H
