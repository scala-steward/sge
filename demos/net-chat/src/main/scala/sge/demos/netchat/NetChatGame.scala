...
import _root_.sge.utils.Millis
import _root_.sge.utils.Nanos
import _root_.sge.utils.Nullable
import _root_.sge.utils.ScreenUtils
import _root_.sge.utils.TextFormatter
import _root_.sge.utils.TimeUtils
import _root_.sge.utils.XmlReader
...
object NetChatGame extends DemoScene {
  ...
  override def init()(using _root_.sge.Sge): Unit = {
  ...
  override def render(dt: Float)(using _root_.sge.Sge): Unit = {
    // Input: TAB to cycle XML samples
    if (_root_.sge.Sge().input.isKeyJustPressed(Input.Keys.TAB)) {
      xmlIndex = (xmlIndex + 1) % XmlSamples.length
      parseCurrentXml()
    }

    // Input: R to re-parse current XML
    if (_root_.sge.Sge().input.isKeyJustPressed(Input.Keys.R)) {
      parseCurrentXml()
    }

    // Input: C to copy to clipboard
    if (_root_.sge.Sge().input.isKeyJustPressed(Input.Keys.C)) {
      val clip = _root_.sge.Sge().application.getClipboard()
      clip.contents = Nullable("SGE Demo Clipboard Test")
    }

    // Input: V to check clipboard
    if (_root_.sge.Sge().input.isKeyJustPressed(Input.Keys.V)) {
      val clip = _root_.sge.Sge().application.getClipboard()
    ...
    val screenH = _root_.sge.Sge().graphics.getHeight().toFloat
  ...
  override def resize(width: _root_.sge.Pixels, height: _root_.sge.Pixels)(using _root_.sge.Sge): Unit = {
    viewport.update(width, height, true)
  }

  override def dispose()(using _root_.sge.Sge): Unit = {
    shapeRenderer.close()
  }

  // --- Card rendering ---

  private def drawCard(baseY: Float, index: Int)(using _root_.sge.Sge): Unit = {
  ...
  private def drawTimeCard(y: Float)(using _root_.sge.Sge): Unit = {
