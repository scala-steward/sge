...
class AngleGL31Native extends AngleGL30Native with GL31 {
  ...
  override def glDrawArraysIndirect(mode: Int, indirect: Long): Unit =
    GL31C.glDrawArraysIndirect(mode, offsetPtr(indirect.toInt))

  override def glDrawElementsIndirect(mode: Int, `type`: Int, indirect: Long): Unit =
    GL31C.glDrawElementsIndirect(mode, `type`, offsetPtr(indirect.toInt))
  ...
  override def glTexStorage2DMultisample(
    ...
    GL31C.glTexStorage2DMultisample(target, samples, internalformat, width, height, glBool(fixedsamplelocations))
  ...
  override def glGetTexLevelParameteriv(target: Int, level: Int, pname: Int, params: IntBuffer): Unit =
    GL31C.glGetTexLevelParameteriv(target, level, pname, bufPtr(params).asInstanceOf[Ptr[CInt]])

  override def glGetTexLevelParameterfv(target: Int, level: Int, pname: Int, params: FloatBuffer): Unit =
    GL31C.glGetTexLevelParameterfv(target, level, pname, bufPtr(params).asInstanceOf[Ptr[CFloat]])
  ...
  override def glVertexAttribFormat(
    ...
    GL31C.glVertexAttribFormat(attribindex, size, `type`, glBool(normalized), relativeoffset)

  override def glVertexAttribIFormat(attribindex: Int, size: Int, `type`: Int, relativeoffset: Int): Unit =
    GL31C.glVertexAttribIFormat(attribindex, size, `type`, relativeoffset)
