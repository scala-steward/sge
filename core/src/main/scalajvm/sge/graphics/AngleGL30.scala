...
class AngleGL30(lookup: SymbolLookup) extends AngleGL20(lookup) with GL30 {
  ...
  override def glUnmapBuffer(target: Int): Boolean = fromGlBool30(_glUnmapBuffer.invoke(target))
  ...
  override def glBeginTransformFeedback(primitiveMode: Int): Unit = _glBeginTransformFeedback.invoke(primitiveMode)
  ...
  override def glBindBufferRange(target: Int, index: Int, buffer: Int, offset: Int, size: Int): Unit =
    _glBindBufferRange.invoke(target, index, buffer, offset, size)
  ...
  override def glBindBufferBase(target: Int, index: Int, buffer: Int): Unit =
    _glBindBufferBase.invoke(target, index, buffer)
