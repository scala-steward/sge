 ...
 *   Convention: LWJGL SharedLibraryLoader/NativesLoader -> direct Panama lib loading
 *   Convention: GLDebugMessageSeverity removed (ANGLE doesn't support ARB/KHR debug)
 *   Convention: loadANGLE/postLoadANGLE removed (SGE loads ANGLE libs at Panama init time)
...
class DesktopApplication(
  ...
  private val glLookup:  java.lang.foreign.SymbolLookup
  ...
  private[sge] def setupWindow(
    ...
    // Initialize GL bindings from the ANGLE libGLESv2 shared library
    val gl32 = sge.graphics.AngleGL32(glLookup)
    ...
    while (i < 2) {
      ...
      window.getGraphics().getGL20().glClear(0x00004000) // GL_COLOR_BUFFER_BIT
...
object DesktopApplication {
  ...
  /** Creates and runs a desktop application.
    *
    * This is the primary entry point for SGE desktop games. It loads the GLFW, ANGLE, and miniaudio shared libraries from `java.library.path`, initializes the windowing/audio/GL subsystems, and
  ...
  def apply(
    ...
    new DesktopApplication(listener, config, windowing, audioOps, glLookup)
  }
