...
class DefaultDesktopInput private[sge] (
  ...
  override def getX(): Int = _mouseX

  override def getX(pointer: Int): Int = if (pointer == 0) _mouseX else 0

  override def getDeltaX(): Int = _deltaX

  override def getDeltaX(pointer: Int): Int = if (pointer == 0) _deltaX else 0

  override def getY(): Int = _mouseY

  override def getY(pointer: Int): Int = if (pointer == 0) _mouseY else 0

  override def getDeltaY(): Int = _deltaY

  override def getDeltaY(pointer: Int): Int = if (pointer == 0) _deltaY else 0
  ...
  override def isButtonPressed(button: Int): Boolean =
    windowing.getMouseButton(window.getWindowHandle(), button) == GLFW_PRESS

  override def isButtonJustPressed(button: Int): Boolean =
    if (button < 0 || button >= _justPressedButtons.length) false
    else _justPressedButtons(button)
