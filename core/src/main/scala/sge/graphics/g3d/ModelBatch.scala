  ...
  def flush(): Unit = {
    camera.foreach { cam =>
      ...
      while (i < renderables.size) {
        ...
        if (currentShader.orNull ne renderable.shader.orNull) {
...
object ModelBatch {
  ...
  protected class RenderablePool extends Pool.Default[Renderable](() => new Renderable()) with Pool.Flushable[Renderable] {
