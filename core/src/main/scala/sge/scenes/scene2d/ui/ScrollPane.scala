...
class ScrollPane(actor: Nullable[Actor], style: ScrollPane.ScrollPaneStyle)(using sge: Sge) extends WidgetGroup with Styleable[ScrollPane.ScrollPaneStyle] {
  ...
  //   this(actor, skin.get(classOf[ScrollPaneStyle]))
  // }

  // /** @param actor May be null. */
  // def this(actor: Nullable[Actor], skin: Skin, styleName: String)(using sge: Sge) = {
  //   this(actor, skin.get(styleName, classOf[ScrollPaneStyle]))
  // }
  ...
  protected def getFlickScrollListener(): ActorGestureListener = {
    ...
    new ActorGestureListener() {
      ...
      override def fling(event: InputEvent, x: Float, y: Float, button: Int): Unit = {
