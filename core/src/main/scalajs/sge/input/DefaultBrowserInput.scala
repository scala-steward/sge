...
class DefaultBrowserInput(canvas: HTMLCanvasElement, config: BrowserApplicationConfig)(using Sge) extends BrowserInput {
  ...
  private val pressedKeys:     Array[Boolean]                    = new Array[Boolean](Keys.MAX_KEYCODE + 1)
  private val justPressedKeys: Array[Boolean]                    = new Array[Boolean](Keys.MAX_KEYCODE + 1)
  private val keysToCatch:     scala.collection.mutable.Set[Int] = scala.collection.mutable.HashSet()
  ...
  private val pressedButtons:        scala.collection.mutable.Set[Int]      = scala.collection.mutable.HashSet()
  private val justPressedButtonsArr: Array[Boolean]                         = new Array[Boolean](5)
  private var _justTouched:          Boolean                                = false
  private var currentEventTimeStamp: Long                                   = 0L
  ...
  override def getX():                  Int = touchXArr(0)
  override def getX(pointer:      Int): Int = touchXArr(pointer)
  override def getDeltaX():             Int = deltaXArr(0)
  override def getDeltaX(pointer: Int): Int = deltaXArr(pointer)
  override def getY():                  Int = touchYArr(0)
  override def getY(pointer:      Int): Int = touchYArr(pointer)
  override def getDeltaY():             Int = deltaYArr(0)
  override def getDeltaY(pointer: Int): Int = deltaYArr(pointer)
  ...
  override def isButtonJustPressed(button: Int): Boolean =
    if (button < 0 || button >= justPressedButtonsArr.length) false
    else justPressedButtonsArr(button)
  ...
  override def isKeyPressed(key: Int): Boolean =
    ...
    else if (key < 0 || key > Keys.MAX_KEYCODE) false
    else pressedKeys(key)
  ...
  override def getCurrentEventTime():                                                        Long  = currentEventTimeStamp
  ...
  override def setCursorPosition(x: Int, y: Int): Unit = () // not possible in browser
  ...
  private def handleMouseDown(e: MouseEvent): Unit = {
    ...
    if (!pressedButtons.contains(button)) {
      ...
      if (button < justPressedButtonsArr.length) justPressedButtonsArr(button) = true
      deltaXArr(0) = 0
      deltaYArr(0) = 0
      if (isCursorCatched()) {
      ...
      if (processor != null) processor.touchDown(touchXArr(0), touchYArr(0), 0, button)
    }
  }
  ...
  private def handleDocMouseDown(e: MouseEvent): Unit =
    ...
    if (e.target != canvas) {
      ...
      if (mx < 0 || mx > Sge().graphics.getWidth() || my < 0 || my > Sge().graphics.getHeight()) {
        hasFocus = false
      }
    }
  ...
  private def handleMouseMove(e: MouseEvent): Unit = {
    ...
    currentEventTimeStamp = utils.TimeUtils.nanoTime()
    if (processor != null) {
      if (touched(0)) processor.touchDragged(touchXArr(0), touchYArr(0), 0)
      else processor.mouseMoved(touchXArr(0), touchYArr(0))
    }
  }
  ...
  private def handleMouseUp(e: MouseEvent): Unit = {
    ...
    if (pressedButtons.contains(button)) {
      ...
      currentEventTimeStamp = utils.TimeUtils.nanoTime()
      touched(0) = false
      if (processor != null) processor.touchUp(touchXArr(0), touchYArr(0), 0, button)
    }
  }
  ...
  private def releaseAllKeys(): Unit = {
    var i = 0
    while (i <= Keys.MAX_KEYCODE) {
      if (pressedKeys(i)) {
        pressedKeys(i) = false
        pressedKeyCount -= 1
        if (processor != null) processor.keyUp(i)
      }
      i += 1
    }
  }
  ...
  private def handleTouchStart(e: TouchEvent): Unit = {
    ...
    while (i < touches.length) {
      ...
      if (touchId >= 0) {
        ...
        deltaXArr(touchId) = 0
        deltaYArr(touchId) = 0
        if (processor != null) processor.touchDown(touchXArr(touchId), touchYArr(touchId), touchId, Buttons.LEFT)
      }
      i += 1
    }
    currentEventTimeStamp = utils.TimeUtils.nanoTime()
  ...
  private def handleTouchMove(e: TouchEvent): Unit = {
    ...
    while (i < touches.length) {
      ...
      touchMap.get(real).foreach { touchId =>
        ...
        touchXArr(touchId) = getRelativeTouchX(touch)
        touchYArr(touchId) = getRelativeTouchY(touch)
        if (processor != null) processor.touchDragged(touchXArr(touchId), touchYArr(touchId), touchId)
      }
      i += 1
    }
    currentEventTimeStamp = utils.TimeUtils.nanoTime()
  ...
  private def handleTouchEnd(e: TouchEvent): Unit = {
    ...
    while (i < touches.length) {
      ...
      touchMap.get(real).foreach { touchId =>
        ...
        touchXArr(touchId) = getRelativeTouchX(touch)
        touchYArr(touchId) = getRelativeTouchY(touch)
        if (processor != null) processor.touchUp(touchXArr(touchId), touchYArr(touchId), touchId, Buttons.LEFT)
      }
      i += 1
    }
    currentEventTimeStamp = utils.TimeUtils.nanoTime()
  ...
  private def getButton(button: Int): Int =
    button match {
      case 0 => Buttons.LEFT
...
object DefaultBrowserInput {
  ...
  def keyForCode(keyCode: Int, location: Int): Int =
    keyCode match {
      ...
      case c if c >= Key0 && c <= Key9 => Keys.NUM_0 + (c - Key0)
      case c if c >= KeyA && c <= KeyZ => Keys.A + (c - KeyA)

      case KeyLeftWindowKey  => Keys.UNKNOWN
      case KeyRightWindowKey => Keys.UNKNOWN

      case c if c >= KeyNumpad0 && c <= KeyNumpad9 => Keys.NUMPAD_0 + (c - KeyNumpad0)
      ...
      case c if c >= KeyF1 && c <= KeyF24 => Keys.F1 + (c - KeyF1)
