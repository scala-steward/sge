...
class WebGL20(val gl: js.Dynamic) extends GL20 {
  ...
  override def glBufferData(target: Int, size: Int, data: Buffer, usage: Int): Unit =
    data match {
      case fb: FloatBuffer => gl.bufferData(target, copy(fb), usage)
      case sb: ShortBuffer => gl.bufferData(target, copy(sb), usage)
      case bb: ByteBuffer  => gl.bufferData(target, copy(bb), usage)
      case null => gl.bufferData(target, size, usage)
  ...
  override def glBufferSubData(target: Int, offset: Int, size: Int, data: Buffer): Unit =
    data match {
      case fb: FloatBuffer => gl.bufferSubData(target, offset, copy(fb))
      case sb: ShortBuffer => gl.bufferSubData(target, offset, copy(sb))
      case bb: ByteBuffer  => gl.bufferSubData(target, offset, copy(bb))
      case _ => throw utils.SgeError.GraphicsError("Can only cope with FloatBuffer, ShortBuffer, and ByteBuffer")
    }
  ...
  override def glDrawElements(mode: Int, count: Int, `type`: Int, indices: Buffer): Unit =
    gl.drawElements(mode, count, `type`, indices.position()) // WebGL uses buffer offset, not client pointer
  ...
  override def glIsEnabled(cap: Int): Boolean =
    gl.isEnabled(cap).asInstanceOf[Boolean]
  ...
  override def glTexImage2D(
    ...
    if (pixels == null) {
      ...
      pixels match {
        case fb: FloatBuffer =>
          gl.texImage2D(target, level, internalformat, width, height, border, format, `type`, copy(fb))
        case ib: IntBuffer =>
          ...
          gl.texImage2D(target, level, internalformat, width, height, border, format, `type`, uint8)
        case bb: ByteBuffer =>
          val int8  = copy(bb)
  ...
  override def glTexSubImage2D(
    ...
    pixels match {
      case fb: FloatBuffer =>
        gl.texSubImage2D(target, level, xoffset, yoffset, width, height, format, `type`, copy(fb))
      case ib: IntBuffer =>
        val int32 = copy(ib)
        val uint8 = new Uint8Array(int32.buffer, int32.byteOffset, int32.byteLength)
        gl.texSubImage2D(target, level, xoffset, yoffset, width, height, format, `type`, uint8)
      case bb: ByteBuffer =>
        val int8  = copy(bb)
        val uint8 = new Uint8Array(int8.buffer, int8.byteOffset, int8.byteLength)
        gl.texSubImage2D(target, level, xoffset, yoffset, width, height, format, `type`, uint8)
