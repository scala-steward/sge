...
class BrowserGraphics(
  ...
  override def getWidth():            Int   = canvas.width
  override def getHeight():           Int   = canvas.height
  override def getBackBufferWidth():  Int   = canvas.width
  override def getBackBufferHeight(): Int   = canvas.height
  ...
  override def setWindowedMode(width: Int, height: Int): Boolean = {
    ...
    if (config.isFixedSizeApplication) setCanvasSize(width, height)
    true
  }
